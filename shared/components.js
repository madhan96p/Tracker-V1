...
const COMPONENTS = (() => {
  ...
  function applyTheme(idx) {
    ...
    try { localStorage.setItem(THEME_KEY, String(idx)); } catch(e) {}
  }
  ...
  function initTheme() {
    let saved = 0;
    try { saved = parseInt(localStorage.getItem(THEME_KEY) || '0'); } catch(e) {}
  ...
  function inject(activeId, title) {
    ...
    if (navEl) navEl.innerHTML = buildNav(activeId);
  ...
  function setStreaks(streaks) {
    ...
    if (tEl && streaks.trade) {
      ...
      tEl.className = 'streak-val' + (s.type === 'win' ? ' streak-win' : s.type === 'loss' ? ' streak-loss' : '');
    }
    if (dEl && streaks.day) {
      const s = streaks.day;
      dEl.textContent = s.label || '—';
      dEl.className = 'streak-val' + (s.type === 'win' ? ' streak-win' : s.type === 'loss' ? ' streak-loss' : '');
    }
  }
  ...
  function showError(msg) {
    ...
    if (!b) {
      ...
      b.id = 'errorBanner'; b.className = 'error-banner';
  ...
  async function refresh() { SHEETS.clearCache(); if (_refreshFn) await _refreshFn(); }

  // ─── CHART DEFAULTS ─────────────────────────────────────────────────────────
  const CHART = {
    get accentColor() { return getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#00d2aa'; },
    get gainColor()   { return getComputedStyle(document.documentElement).getPropertyValue('--gain').trim()   || '#4ade80'; },
    get lossColor()   { return getComputedStyle(document.documentElement).getPropertyValue('--loss').trim()   || '#f87171'; },
    get skyColor()    { return getComputedStyle(document.documentElement).getPropertyValue('--sky').trim()    || '#38bdf8'; },
    get warnColor()   { return getComputedStyle(document.documentElement).getPropertyValue('--warn').trim()   || '#fbbf24'; },
    ...
    axes() {
      return {
        ...
        ticks:  { color: '#3d6572', font: { family: "'JetBrains Mono', monospace", size: 10 } },
    ...
    destroy(key) {
      if (window._ftCharts && window._ftCharts[key]) {
        try { window._ftCharts[key].destroy(); } catch(e) {}
  ...
  function init(activeId, title) { inject(activeId, title); }

  return {
    init, setConn, setLastSync,
    showLoader, hideLoader, showError, hideError,
    openSidebar, closeSidebar,
    setStreaks, cycleTheme, applyTheme,
    onRefresh, refresh,
    CHART,
  };
})();
