...
function renderKPIs(foRows, holdRows, logRows, streaks) {
  ...
  const kpis = [
    ...
    },
    {
      label: "Win Rate",
      value: fo.winRate + "%",
      cls: +fo.winRate >= 50 ? "gain" : "loss",
      card: +fo.winRate >= 50 ? "c-gain" : "c-loss",
    ...
    },
    {
      ...
      sub: logRows.length + " log entries",
    },
    {
    ...
    },
    {
      label: "📈 Trade Streak",
      value: ts.label || "—",
      cls: ts.type === "win" ? "gain" : ts.type === "loss" ? "loss" : "",
      card:
        ts.type === "win" ? "c-gain" : ts.type === "loss" ? "c-loss" : "c-sky",
      sub: "Consecutive trades",
    },
    {
      label: "📅 Day Streak",
      value: ds.label || "—",
      cls: ds.type === "win" ? "gain" : ds.type === "loss" ? "loss" : "",
      card:
        ds.type === "win" ? "c-gain" : ds.type === "loss" ? "c-loss" : "c-sky",
      sub: "Consecutive trading days",
    },
  ];
...
function renderCumPnlChart(rows) {
  try {
      ...
      new Chart(ctx, {
          ...
          datasets: [
            {
              ...
              backgroundColor: (c2) => {
                const gg = c2.chart.ctx.createLinearGradient(0, 0, 0, 280);
                gg.addColorStop(0, "rgba(0,210,170,0.18)");
                gg.addColorStop(1, "rgba(0,210,170,0.01)");
                return gg;
              },
              borderWidth: 2,
              fill: true,
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              ...K.tooltip,
              callbacks: { label: (c) => " " + UTILS.pnl(c.raw) },
            },
          },
    ...
    console.error("[Dashboard] cumPnl:", e);
  }
}
...
function renderWinLossChart(rows) {
  try {
    ...
    console.error("[Dashboard] winLoss:", e);
  }
}
...
function renderDailyPnlChart(rows) {
  try {
    ...
    console.error("[Dashboard] dailyPnl:", e);
  }
}
...
function renderHoldingsSnap(rows) {
  ...
  const sorted = [...(rows || [])]
    .sort(
      (a, b) =>
        UTILS.toNum(g(b, "Current Value", 0)) -
        UTILS.toNum(g(a, "Current Value", 0)),
    )
    .slice(0, 8);
  const tbody = document.getElementById("holdingsSnap");
  if (!sorted.length) {
    tbody.innerHTML =
      '<tr class="empty-row"><td colspan="8">No holdings data</td></tr>';
    return;
  }
  tbody.innerHTML = sorted
    .map((row) => {
      ...
      const signal =
        g(row, "Master Sentinel", g(row, "Sentinel Recommendation", "")) || "";
      return `<tr>
